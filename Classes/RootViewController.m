...
- (void)viewDidLoad
{
  ...
  CGRect arect = CGRectInset(footerView.bounds, 50, 20);
  CGRect frame1 = CGRectOffset(arect, 0, -20);
  CGRect frame2 = CGRectOffset(arect, 0, 20);
  NSLog(@"%@,%@,%@",NSStringFromCGRect(arect),NSStringFromCGRect(frame1),NSStringFromCGRect(frame2));
  
  [aboutButton setFrame:frame1];
  ...
  UIButton *aboutButton2 = [UIButton buttonWithType:UIButtonTypeCustom];
  [aboutButton2 setTitle:NSLocalizedString(@"Refresh", nil) forState:UIControlStateNormal];
  [aboutButton2 setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
  aboutButton2.titleLabel.font = [UIFont systemFontOfSize:14.0];
  aboutButton2.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
  aboutButton2.showsTouchWhenHighlighted = YES;
  [aboutButton2 setFrame:frame2];
  [aboutButton2 addTarget:self action:@selector(refresh:) forControlEvents:UIControlEventTouchUpInside];
  [footerView addSubview:aboutButton2];
...
{
  DocSetDownloadManager * a = [DocSetDownloadManager sharedDownloadManager];
  [a reloadDownloadedDocSets];
  [self.tableView reloadData];
